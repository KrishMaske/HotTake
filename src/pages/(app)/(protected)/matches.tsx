 ...
 * participant in before it reaches the socket. Filtering here would be
 * decoration.
 */
...
import { otherParticipant, relativeTime, type Profile } from '../../../lib/hottake'
...
export default function Matches() {
  ...
  const profileByUser = new Map(profiles.map((p) => [p.data.userId, p]))
  ...
  return (
    <div className="px-5 py-6">
      ...
      {matches.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3" data-testid="matches-list">
          {matches.map((match) => {
            const themId = userId ? otherParticipant(match.data, userId) : undefined
            const them = themId ? profileByUser.get(themId) : undefined
            ...
            return (
              <li key={match.recordId}>
                <Link
                  ...
                  className="flex items-center gap-4 rounded-2xl border border-border bg-card p-3 transition-colors hover:border-primary"
                >
                  <img
                    src={photoUrl(them?.data.photoKey)}
                    alt=""
                    ...
                    <p className="mt-0.5 truncate text-sm text-muted-foreground">
                      {latest ? (
                        ...
                        <>
                          ...
                          {them?.data.hotTake ?? 'Say something controversial.'}
                        </>
                      )}
                    </p>
                  </div>
